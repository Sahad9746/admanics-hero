...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Read ${post.title} on Admanics Blog.`,
    openGraph: {
      title: post.title,
      description: `Read ${post.title} on Admanics Blog.`,
    ...
    <main className="min-h-screen bg-neutral-950 pt-32 pb-20">
      <div className="max-w-4xl mx-auto px-6">
        <div className="mb-10 text-center">
          {post.categories?.length > 0 && (
            <div className="flex items-center justify-center gap-2 mb-4">
              {post.categories.map((category: string) => (
                <span
                  key={category}
                  className="text-sm font-medium text-blue-500 uppercase tracking-widest"
                >
                  {category}
                </span>
              ))}
          ...
          <h1 className="text-heading-lg text-white mb-6">{post.title}</h1>
